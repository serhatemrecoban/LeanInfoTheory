#!/usr/bin/env python3
"""Generate the current LeanInfoTheory public API inventory.

Reuse the maintained declaration parser and module classifier. Counts describe
current source; the immutable release inventory is a separate baseline. This
source inventory does not replace the retained-type gate or Lean-generated
signature-bearing documentation.
"""
...
from frozen_public_api import baseline_identity, verify_frozen_manifest


ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT / "docs" / "current-public-api.json"
...
def build_manifest() -> dict[str, object]:
    ...
    return {
        "schema": "lean-info-theory.current-public-api.v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"current public API generation failed: {exc}", file=sys.stderr)
        return 1
    if args.check:
        ...
        if OUTPUT.read_bytes() != rendered.encode("utf-8"):
            print(
                "current public API manifest is stale; run "
                "python scripts/generate_current_public_api.py"
            )
            return 1
        print("current public API manifest matches the source")
        return 0

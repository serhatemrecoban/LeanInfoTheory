...
This is not a replacement for Lean doc-gen. It records the non-private
declarations we can reliably extract from local Lean source files, together
with module names, declaration kinds, source locations, and nearby doc comments.
"""
...
def generated_json(declarations: list[Declaration]) -> str:
...
def write_json(declarations: list[Declaration]) -> None:
    JSON_OUTPUT.write_text(generated_json(declarations), encoding="utf-8", newline="\n")
...
def metric_cards(declarations: list[Declaration]) -> str:
    ...
    return f"""
        <div class="metric-grid">
          <div class="metric-card"><strong>{len(declarations)}</strong><span>non-private source declarations</span></div>
    ...
    <main class="main-content" role="main">
      <section class="status-strip" aria-label="Generation status">
        <p>
          ...
          local Lean source files. This supplementary inventory includes
          non-stable examples and reference anchors. Source links target current
          development on <code>master</code>; dirty local lines may differ from
          that branch. Maintenance staging pins these links to its exact site
          commit. Current supported coverage is recorded in
          <code>docs/current-public-api.json</code>. The separate
          <a href="v0.1.0/">frozen v0.1.0 signature-bearing reference</a>
          documents the released 601 declarations in 31 modules.
        </p>
      </section>
...
def write_html(declarations: list[Declaration]) -> None:
    HTML_OUTPUT.write_text(generated_html(declarations), encoding="utf-8", newline="\n")
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="fail unless both checked-in artifacts match the current source",
    )
    args = parser.parse_args()

    declarations = all_declarations()
    rendered_json = generated_json(declarations)
    rendered_html = generated_html(declarations)

    if args.check:
        ok = check_output(JSON_OUTPUT, rendered_json)
        ok = check_output(HTML_OUTPUT, rendered_html) and ok
        if ok:
            print("website API index artifacts are current")
            return 0
        return 1

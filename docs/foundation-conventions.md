...
- Basic PMF facts such as `PMF.coe_le_one`, `PMF.tsum_coe`, and
  `PMF.apply_ne_top`.
- Local real-mass bridge lemmas are also placed in the `PMF` namespace, e.g.
  `PMF.toReal_nonneg`, `PMF.toReal_le_one`, and `PMF.sum_toReal`, matching
  mathlib's convention for object-specific lemmas.
...
- `entropy` agrees with the finite expectation of `log (1 / p(X))`.
...
- Entropy is invariant under equivalences and injective relabelings.
- Joint entropy is invariant under product reassociation and coordinate swap.
